(function () {
	...
	return {
		events: {
			'app.activated': 'showCreatePaymentForm'
			, 'click #create-payment-btn': 'createPaymentSubmit'
		}
		, requests: {
			createEmail: function (toSend) {
				return {
					url: 'https://khipu.com/api/1.3/createEmail',
					type: 'POST',
					dataType: 'json',
					data: toSend
				};
		...
		, showCreatePaymentForm: function () {
			...
			this.switchTo('create_payment_form', data);
		}

		,
		createPaymentSubmit: function (event) {
			event.preventDefault();
			var receiver_id = this.settings.receiverId;
			var secret = this.settings.secret;
			var amount = this.$('#amount')[0].value;
			var subject = this.$('#subject')[0].value;
			var body = this.$('#body')[0].value;
			var name = this.$('#name')[0].value;
			var email = this.$('#email')[0].value;
			var pay_directly = 'false';
			...
			var destinataries = '[{"name":"' + name + '", "email": "' + email + '", "amount": "' + amount + '"}]';
			...
			this.ajax('createEmail', toSend).done(function (data) {
				...
				services.notify(data.responseJSON.error.message, 'error');
			});
